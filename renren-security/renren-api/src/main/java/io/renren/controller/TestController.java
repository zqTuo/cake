...
public class TestController {
    ...
    public R get() throws IOException {
        ...
        for (Driver driver:driverList){
            ...
            StringBuilder driverNewFileName = new StringBuilder(driverPreName); //新驾照模板文件名 CA_0.jpg
            ...
            while (judeFileExists(new File(driverPath + "/" + driverNewFileName + "_" + temp + driver_suffix))){ // 如果文件存在 就加一
            ...
            if(!isExist(new File(newFilePath))){
            ...
            if(!StringUtils.isNullOrEmpty(driver.getForgeryPic())){
                ...
                StringBuilder forgeryNewFileName = new StringBuilder(forgery_suffix); //新驾照防伪图片文件名 CA_m_0.jpg
                if(!StringUtils.isNullOrEmpty(driverPro)){
                    forgeryNewFileName.append("_").append(driverPro);
                }
                logger.info("新驾照防伪名称路径：" +driverPath + "/" +  driverNewFileName  + "_m_" + temp1 + driver_suffix);
                while (judeFileExists(new File(driverPath + "/" + driverNewFileName + "_m_" + temp1 + driver_suffix))){ // 如果文件存在 就加一
                    logger.info("该路径已存在，数量+1");
                    temp1++;
                }

                forgeryNewFileName.append("_").append(temp1).append(forgery_suffix);
