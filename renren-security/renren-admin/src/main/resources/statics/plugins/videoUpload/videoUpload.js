...
function sub(){
	...
	var url = "/cake-admin/admin/upload/videoUpload"; // 接收上传文件的后台地址
	...
	if(evt.target.responseText){
		...
		if(result.code === 0){
			...
			if($(".preview video source").length > 0){
				...
				$(".preview video source").attr('src',url);
			}else{
				$(".preview").append("<video  controls='' autoplay='' name='media' style='max-width: 380px;'><source src="+ url +" type='video/mp4'></video>");
			}
			$(".preview").attr('data-videoSrc',url);
		}else{
